...
class TradingStrategy:
    ...
    def execute_strategy(self):
        ...
        current_price = kraken_api.get_XRP_price()
        if current_price is None:
            logger.error("Failed to retrieve XRP price.")
            return
        ...
        logger.info(f"Current XRP Price: {current_price}, Moving Average: {moving_avg}, RSI: {rsi}, MACD: {macd}, Signal: {signal}, Sentiment Score: {self.sentiment_score}")
    ...
    def _execute_buy(self, current_price: float):
        ...
        if self.last_trade_type != 'buy' and (potential_profit_loss is None or is_profitable_trade(potential_profit_loss)):
            logger.info(colored(f"Buying XRP... Signal: MACD crossover above SignalRSI < 40 (moderately oversold), Potential Profit: {potential_profit_loss if potential_profit_loss else 0:.2f}%, Market Volume: {market_volume}", 'green'))
    ...
    def _execute_partial_sell(self, current_price: float):
        ...
        if self.last_trade_type != 'sell' and (potential_profit_loss is None or is_profitable_trade(potential_profit_loss)):
            logger.info(colored(f"Partially selling XRP...Signal: MACD crossover below SignalRSI > 60 (moderately overbought), Potential Profit: {potential_profit_loss if potential_profit_loss else 0:.2f}%", 'yellow'))
